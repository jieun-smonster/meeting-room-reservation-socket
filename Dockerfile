...
HEALTHCHECK --interval=30s --timeout=30s --start-period=5s --retries=3 \
  CMD python -c "import os; print('Health check passed' if all([os.getenv(key) for key in ['SLACK_BOT_TOKEN', 'SLACK_APP_TOKEN', 'NOTION_API_KEY']]) else exit(1))"

# 8. 컨테이너가 시작될 때 실행할 기본 명령어 설정
